...
def main():
    ...
    if(args.act_mode_8bit):
        print("Running in 8 bit mode.")
        
        if not evaluate_only:
            print("WARNING Running in 8 bit mode but training enabled")
        
    if(evaluate_only):
       use_load_patch = True
       print("Evaluation only, training disabled")
...
def patch_attack(args,image, applied_patch, mask, target, probability_threshold, model, lr=1, max_iteration=100, fastsign = False, momentum = False , decay_factor = 0 ):
    ...
    while target_probability < probability_threshold and count < max_iteration:
        ...
        #       # patch_grad = torch.randint(-1,2,patch_grad.size())
        if(fastsign):
            patch_grad = patch_grad.sign()
        #follow gradient
